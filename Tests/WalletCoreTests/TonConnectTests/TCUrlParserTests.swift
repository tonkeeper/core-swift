...
final class TCUrlParserTests: XCTestCase {
    ...
    func testDemoDappWithReactUITCUrlParse() throws {
        ...
        let parser = TCUrlParser()
        let parameters = try parser.parseUrl(url)
...
extension TCRequestPayload.Item: Equatable {
    public static func == (lhs: TCRequestPayload.Item, rhs: TCRequestPayload.Item) -> Bool {
        switch (lhs, rhs) {
